...
namespace TOEICWEB.Controllers
{
    ...
    public class LoTrinhController : ControllerBase
    {
        ...
        [Authorize]
        [HttpGet("co-san/{maLoTrinh}/bai-hoc-noi-dung")]
        public async Task<IActionResult> GetBaiHocTheoLoTrinhNoiDung(string maLoTrinh)
        {
            try
            {
                var maNd = GetMaNd();
                ...
                // Danh sách bài học thuộc lộ trình
                ...
                if (!lessons.Any())
                {
                    return Ok(new
                    {
                        message = "Lộ trình chưa có bài học",
                ...
                // Tất cả bài đọc thuộc các bài học + trạng thái hoàn thành theo user
                ...
                // Tất cả bài nghe thuộc các bài học + trạng thái hoàn thành theo user
                ...
                // Gộp theo bài học + tính trạng thái hoàn thành bài học
                ...
                var soBaiHoanThanh = result.Count(r => r.DaHoanThanhBaiHoc);

                return Ok(new
                {
                    message = "Danh sách bài học theo lộ trình kèm trạng thái hoàn thành",
                    maLoTrinh = loTrinh.MaLoTrinh,
                    tenLoTrinh = loTrinh.TenLoTrinh,
                    kyNangTrongTam = loTrinh.KyNangTrongTam,
                    chuDeBaiHoc = loTrinh.ChuDeBaiHoc,
                    tongSoBaiHoc = result.Count,
                    soBaiHoanThanh,
                    data = result
                });
            }
